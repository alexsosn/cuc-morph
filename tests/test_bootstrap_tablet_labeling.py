...
def _init_bootstrap_schema(conn: sqlite3.Connection) -> None:
    ...
    cur.execute(
        ...
        "entry_id INTEGER PRIMARY KEY, lemma TEXT, homonym TEXT, pos TEXT, text TEXT)"
    )
    cur.execute(
...
class BootstrapTabletLabelingTest(unittest.TestCase):
    ...
    def test_load_dulat_forms_generates_weak_final_prefixed_alias(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "dulat.sqlite"
            conn = sqlite3.connect(db_path)
            _init_bootstrap_schema(conn)
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO entries(entry_id, lemma, homonym, pos) VALUES (1, '/ġ-l-y/', '', 'vb')"
            )
            cur.execute("INSERT INTO forms(entry_id, text) VALUES (1, 'tġly')")
            cur.execute("INSERT INTO translations(entry_id, text) VALUES (1, 'to wither')")
            conn.commit()
            conn.close()

            forms_map = load_dulat_forms(db_path)
            self.assertIn("tġl", forms_map)
            ids = {e.entry_id for e in forms_map["tġl"]}
            self.assertEqual(ids, {1})

    def test_load_dulat_forms_uses_forms_block_fallback_from_entry_text(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "dulat.sqlite"
            conn = sqlite3.connect(db_path)
            _init_bootstrap_schema(conn)
            ...
            cur.executemany(
                "INSERT INTO forms(entry_id, text) VALUES (1, ?)",
                [("ġly",), ("yġly",)],
            )
            cur.execute("INSERT INTO translations(entry_id, text) VALUES (1, 'to wither')")
            conn.commit()
    ...
    def test_load_dulat_forms_does_not_index_word_break_suffix_as_standalone_form(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "dulat.sqlite"
            conn = sqlite3.connect(db_path)
            _init_bootstrap_schema(conn)
            ...
            cur.execute("INSERT INTO forms(entry_id, text) VALUES (1, 'ytn')")
            cur.execute("INSERT INTO translations(entry_id, text) VALUES (1, 'to give')")
            conn.commit()
            conn.close()

            forms_map = load_dulat_forms(db_path)
            self.assertIn("ytnhm", forms_map)
            self.assertIn("ytnnn", forms_map)
