...
class LintOutputParserTest(unittest.TestCase):
    def test_parse_counts_and_normalization(self):
        ...
        buckets = {(row.severity, row.problem_type): row.count for row in stats.by_problem_type}
        ...
        self.assertEqual(buckets[("INFO", "Surface parsed inconsistently across IDs")], 1)
