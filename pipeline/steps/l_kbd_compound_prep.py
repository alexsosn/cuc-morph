"""Normalize `l + kbd(I)` compound-preposition usage."""
...
def _is_kbd_i_row(row: TabletRow) -> bool:
    return (
        row.surface.strip() == "kbd"
        and row.analysis.strip() == "kbd(I)/"
        and row.dulat.strip() == "kbd (I)"
    )
...
def _forced_kbd_compound_row(source: TabletRow) -> TabletRow:
    return TabletRow(
        line_id=source.line_id,
        surface=source.surface,
        analysis="kbd(I)/",
...
class LKbdCompoundPrepDisambiguator(RefinementStep):
    """Force `l(I)` + `kbd(I)` compound-preposition parsing in `l kbd` sequences."""

    @property
    def name(self) -> str:
        return "l-kbd-compound-prep"
    ...
    def refine_file(self, path: Path) -> StepResult:
        ...
        for idx, group in enumerate(groups):
            ...
            if next_group is None or next_group.key[1] != "kbd":
                continue
            if not any(_is_kbd_i_row(row) for row in next_group.rows):
                continue
            ...
            kbd_target_indexes = [
                ...
                if _is_kbd_i_row(row)
            ]
            keep_kbd_index = kbd_target_indexes[0] if kbd_target_indexes else next_group.indexes[0]
            forced_kbd = _forced_kbd_compound_row(parsed_rows[keep_kbd_index])
            if forced_kbd.to_tsv() != parsed_rows[keep_kbd_index].to_tsv():
                replace_rows[keep_kbd_index] = forced_kbd
            remove_indexes.update(
                row_index for row_index in next_group.indexes if row_index != keep_kbd_index
            )
